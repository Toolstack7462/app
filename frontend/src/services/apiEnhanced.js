...
// ============================================================================
// UNIVERSAL API CONFIGURATION - ENVIRONMENT AGNOSTIC (Enhanced Version)
// ============================================================================
...
function getApiBaseUrl() {
  ...
  if (envBackendUrl) {
    try {
      ...
      if (envOrigin !== currentOrigin && !envBackendUrl.includes('localhost')) {
        console.log('[API Enhanced] Using configured backend URL:', envBackendUrl);
  ...
  console.log('[API Enhanced] Using same-origin relative path: /api/crm');
  return '/api/crm';
}
...
api.interceptors.request.use(
  (config) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[API Enhanced] ${config.method?.toUpperCase()} ${config.baseURL}${config.url}`);
    }
